...
public class ProyectoServiceIntegrationTest {
    ...
    void crearProyectoConTareasYProfesionalesAsignados() {
        ...
        proyectoDTO.setNombre("Proyecto Integración");
        proyectoDTO.setDescripcion("Verifica que un proyecto se guarde con sus tareas asignadas");
        proyectoDTO.setEstado("EN_PROGRESO");
        proyectoDTO.setFechaInicio("2026-05-06");
        proyectoDTO.setFechaFin("2026-10-01");
        proyectoDTO.setComentarios("Proyecto de prueba con tareas y usuarios asignados");

        proyectoService.add(proyectoDTO);
        ...
        proyectoService.addTarea(proyectoGuardado.getId(), tarea1);
        proyectoService.addTarea(proyectoGuardado.getId(), tarea2);

        Proyecto saved = proyectoRepository.findById(proyectoGuardado.getId()).orElseThrow();

        assertThat(saved.getNombre()).isEqualTo("Proyecto Integración");
        assertThat(saved.getTareasDelProyecto()).hasSize(2);

        assertThat(saved.getTareasDelProyecto())
            .extracting(Tarea::getNombre)
        ...
        assertThat(saved.getTareasDelProyecto())
            .extracting(Tarea::getIdProfesionalAsignado)
            .containsExactlyInAnyOrder(101L, 102L);
    }

    @Test
    void agregarTareaAProyectoExistenteConUsuarioAsignado() {
        CreateProyectoDTO proyecto = new CreateProyectoDTO();
        ...
        Long projectId = proyectoRepository.findAll().get(0).getId();
